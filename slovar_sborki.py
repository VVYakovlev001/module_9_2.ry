...
second_result = {x:y for x in first_strings for y in second_strings
                 if len(x) == len(y)}
third_result = {(first_string,len(second_string)) for first_string in first_strings
...
# парой ключ-значение будет строка-длина строки. Значения строк будут
...
#{'Elon': 4, 'Musk': 4, 'Programmer': 10, 'Monitors': 8, 'Variable': 8,
# 'Task': 4, 'Java': 4, 'Computer': 8}
